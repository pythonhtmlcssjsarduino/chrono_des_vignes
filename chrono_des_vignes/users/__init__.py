...
def login():
    ...
    if form.validate_on_submit():
        user: User | None = (
            ...
            .filter_by(username=assert400(form.username.data))
            .first()
        )

        if user and bcrypt.check_password_hash(
            user.password, assert400(form.password.data)
        ):
            login_user(user)
...
def signup() -> str | Response:
    ...
    if form.validate_on_submit():
        ...
        user = User(
            name=assert400(form.name.data).strip(),
...
@set_route(
    users, "/<event_name>/edition/<edition_name>/inscription", methods=["POST", "GET"]
)
def inscription_page(event_name: str, edition_name: str) -> str | Response:
    ...
    if user:
        ...
        if form.validate_on_submit():
            ...
            username = (
                f"{assert400(form.name.data)[:10]}.{assert400(form.lastname.data)[:10]}"
            )
            nb = db.session.query(User).filter(User.username == username).count()
            username += f"({nb})" if nb > 0 else ""
            user = User(
                name=assert400(form.name.data),
...
def modify_profil() -> str | Response:
    ...
    if form.validate_on_submit():
        if (
            ...
            or not User.query().filter_by(name=form.username.data).first()
        ):
    ...
    if form.validate_on_submit():
        if bcrypt.check_password_hash(user.password, assert400(form.old_pwd.data)):  # type: ignore[arg-type]
            hash_pwd = bcrypt.generate_password_hash(
                assert400(form.password.data)
            ).decode("utf-8")  # type: ignore[arg-type]
