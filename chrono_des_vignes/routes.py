...
from chrono_des_vignes import LANGAGES, db, set_route
...
def home() -> str:
    ...
    if current_user.is_authenticated:
        user = current_user
        inscriptions = user.inscriptions.filter(
            Inscription.edition.has(Edition.edition_date > datetime.now())
        ).all()
        participations = user.inscriptions.filter(
            Inscription.edition.has(Edition.edition_date <= datetime.now())
        ).all()
        form = NewEventForm()
    else:
    ...
    next_events = (
        ...
        .filter(
            Event.editions.any(
                and_(Edition.edition_date >= date, Edition.last_inscription >= date)
            )
        )
        .all()
    )
...
DOC_DIR = Path("static/doc")


@main.route("/doc/<path:path>")
@main.route("/doc/<lang>/<path:path>")
@main.route("/doc/")
def doc(path: str = "", lang: str = ""):
    ...
    return send_from_directory(
        DOC_DIR.as_posix(), file.as_posix(), download_name=file.name
    )


@main.route("/doc/assets/<path:path>")
def assets_doc_files(path: str) -> Response:
    return doc_file("assets", path)


@main.route("/doc/search/<path:path>")
...
def doc_file(dir: str, path: str) -> Response:
    return send_from_directory(
        DOC_DIR.as_posix(), f"{dir}/{path}", download_name=Path(path).name
    )
