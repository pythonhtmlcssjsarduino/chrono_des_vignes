...
class Node:
    ...
    def width(self) -> float:
        if len(self.children) == 0:
            return self._width
        return (
            sum([child.width for child in self.children])
            + (len(self.children) - 1) * spacing_x
        )
...
def get_routes(app: Flask) -> list[EndpointData]:
    ...
    for rule in app.url_map.iter_rules():
        if (
            "static" in rule.endpoint
            or "<lang>" in str(rule)
            or rule.subdomain == "dev"
        ):
            continue
        routes.append(
            EndpointData(url=str(rule), endpoint=rule.endpoint, methods=rule.methods)
        )
    return routes
...
def insert_route(
    tree: dict[str, TreeNode], parts: list[str], route: EndpointData
) -> None:
    if not parts:
        return
...
def render_tree_xml(
    ...
    for node in tree:
        ...
        size = 8
        value = (
            f'<font style="font-size: {size}px;">{node.endpoint}<br></font>{escape(node.name)}'
            if node.endpoint
            else escape(node.name)
        )
        ...
        geo = SubElement(
            cell,
            "mxGeometry",
            x=str(x + (node.width + node._width) / 2),
            y=str(y),
            width=str(node._width),
            height=str(Node._height),
        )
        geo.set("as", "geometry")
        ...
        if parent_id != "root":
            ...
            arrow = SubElement(
                parent_element,
                "mxCell",
                style="edgeStyle=none;html=1;entryX=0.5;entryY=0;entryDx=0;entryDy=0;exitX=0.5;exitY=1;exitDx=0;exitDy=0;shape=flexArrow;",
                edge="1",
                source=parent_id,
                target=node_id,
            )
        ...
        render_tree_xml(
            node.children, parent_element, node_id, x, y + node._height + spacing_y
        )
