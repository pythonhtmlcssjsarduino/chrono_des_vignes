...
export class EditionController {
  ...
  constructor(private map: ParcoursMap, private data: ParcoursData) {
    const clk = this.click.bind(this)
    this.map.map.on('click', clk)
    console.log(() => this.map.map.off('click', clk));
  ...
  click(event: LeafletMouseEvent) {
    if (this.creating_first_marker) {
      ...
      const trace = this.extended_segment.data.trace.concat([[this.extended_segment.end.data.lat, this.extended_segment.end.data.lng]])
      this.extended_segment.end.data.setLatLng(event.latlng.lat, event.latlng.lng)
      this.extended_segment.data.trace = trace
      //this.extended_segment.data.trace
    }
  }
  ...
  modifyStand(stand: StandData) {
    ...
    render(panel, this.map.standEdit)
  }
}
...
export class SegmentController {
  ...
  constructor(private map: ParcoursMap, public data: SegmentData, markers: { [k: number]: MarkerController }) {
    if (data.id == 109) {
    ...
    data.on('traceChanged', (trace) => {
      const polypoints = [[start.lat, start.lng], ...(trace.map((point) => [point[0], point[1]])), [end.lat, end.lng]] as [number, number][]
      this.polyline.setLatLngs(polypoints)
      this.refresh(polypoints)
    })

    this.refresh(polyPoints)
  }

  refresh(polyPoints: [number, number][]) {

    this.map.tmp_layer.clearLayers()
    ...
    this.polyline.on('click', () => { this.map.data.selectSegment(this.data.id) })
  ...
  static create(map: ParcoursMap, start: MarkerController, end: MarkerController, index: number) {
    ...
    const controller = new SegmentController(map, segment, { [start.data.id]: start, [end.data.id]: end })
