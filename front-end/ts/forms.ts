...
import formCss from 'inline:../css/form.css'
...
export type Field=FieldGroup|StringField|TextField|FileField
...
export class LitForm extends LitElement {
    ...
    @property({ attribute: false }) onSubmit: (event: Event) => void = () => {}
    ...
    renderStringField(field: StringField):HTMLTemplateResult {
        return html`
            <div class="col-span-full">
                ...
                <div>
                    <input id="${field.name}" type="text" name="${field.name}" autocomplete="${ifDefined(field.autocomplete)}" value="${ifDefined(field.value)}" class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"/>
                </div>
            </div>
        `
    }
    ...
    renderFieldGroup(field: FieldGroup):HTMLTemplateResult {
        ...
        <div class="border-b border-gray-900/10 pb-12">
            ...
            ${field.description==''?html``:html`<p class="mt-1 text-sm/6 text-gray-600">${field.description}</p>`}
