...
export type Events= {[K in Changes['op']]: (change: Extract<Changes, { op: K }>) => void;}&{
...
type Changes = changeSegmentModif|changeStandModif
const linearChanges:Changes['op'][] = []
// #endregion
    ...
    private async commitOp(op:Changes) {
        ...
        if (linearChanges.includes(op.op)) {
            ...
            if (index != -1) {
                ...
                for (const [key, value] of Object.entries(op) as [keyof Changes, never][]) {
                    if (value != undefined){
...
export class SegmentData {
    _trace: [number, number, number|null][]
    eventEmitter=createNanoEvents<segmentEvent>()

    private selected: boolean=false

    constructor(private parcours:ParcoursData, readonly id:number, readonly start:number, readonly to:number, trace:SegmentPoint[],readonly index:number){
        this._trace = trace
        parcours.on('stand:selected', id => {
            if (this.selected){
                this.selected=false
